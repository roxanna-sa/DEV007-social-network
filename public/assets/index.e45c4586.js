...
const createUser = (userMail, userPass, displayName) => createUserWithEmailAndPassword(auth, userMail, userPass).then((userCredential) => {
...
const signIn = (userMail, userPass) => signInWithEmailAndPassword(auth, userMail, userPass);
const signInGoogle = () => {
  ...
  parentElement.addEventListener("click", (event) => {
    ...
    if (target.matches("#loginButton")) {
      ...
      if (userMail === "" || userPass === "") {
        alert("Ingresa email y contrase\xF1a");
      } else {
        signIn(userMail, userPass).then((response) => {
          ...
          alert("Email o contrase\xF1a incorrectos");
        });
      }
...
const Register = (onNavigate2) => {
  ...
  <div class="div-pathology">
    <label for='pathology'>\xBFTienes alguna patolog\xEDa?:</label>
...
const createPost = async (text, files) => {
  const newPost = await pf(_h(db, "posts"), {
    postContent: text,
    user: auth$1.currentUser.email,
    userName: auth$1.currentUser.displayName,
    timestamp: Gf()
  });
...
const addLike = async (postId) => {
  ...
  const userRef = gh(db, "users", auth$1.currentUser.uid);
  try {
    await gf(postRef, {
      likedBy: Qf(userRef)
    });
...
const getPosts = () => {
  const postRef = _h(db, "posts");
  const q2 = Rl(postRef, xl("timestamp", "desc"));
  return df(q2).then((res) => {
    const postsArray = [];
    res.forEach((doc) => {
      const data = doc.data();
      data["id"] = doc.id;
...
const deletePost = async (postId) => {
  await yf(gh(db, "posts", postId));
};
const editPost = async (postId, editInput) => {
  const postRef = gh(db, "posts", postId);
  await gf(postRef, {
    ...
    if (user) {
      ...
      modalPost.innerHTML = `
        <div class='modal-content'>
          <textarea class='postInput ph-center' id='postInput' placeholder= "\xBFQu\xE9 cocinas hoy?"></textarea>
      ...
      async function showAllPosts() {
        ...
        arrayPosts.forEach((post) => {
          ...
          let isLiked = "";
          if (post.likedBy !== void 0) {
            post.likedBy.forEach((like) => {
              if (like.id === auth$1.currentUser.uid) {
          ...
          if (user === post.user) {
            ...
            MenuButton.addEventListener("click", () => {
              ...
              if (clickCount % 2 === 1) {
                let hideModal2 = function() {
                  const modal2 = document.getElementById("modal");
                  modal2.style.display = "none";
                };
                var hideModal = hideModal2;
                  ...
                  if (!deletePostModal) {
                    ...
                    deletePostModal.innerHTML = `
                      <div class="delete-modal-content" id='delete-modal-content'>
                        <p>\xBFDeseas eliminar el post?</p>
                        <div class='modal-btns'>
                ...
                editPostButton.addEventListener("click", async (event) => {
                  ...
                  modalContent2.appendChild(editInput);
                  modalContent2.appendChild(editButton);
        ...
        Array.from(document.getElementsByClassName("likeButton")).forEach((el2) => {
          el2.addEventListener("click", async (clickedElement) => {
      ...
      notLoggedUser.innerHTML = `
        <h2> Bienvenido a Nutrivid, inicia sesi\xF3n o reg\xEDstrate </h2>
